...
class Player {
  ...
  setAction(sublevel) {
    if (
      !(keys.left || keys.right || keys.down || keys.up) &&
  ...
  handleYObstacles(step, sublevel) {
    ...
    audio.handleWalkingOnSounds(
      sublevel.obstacleAt(this.newPos, this.size, this.buffer)
    );
  }
  ...
  touchedItem(item, sublevel) {
    ...
    const addHealth = amount => {
      ...
      status.lifeMeter =
        amount + status.lifeMeter > 10 ? 10 : status.lifeMeter + amount;
    };
  ...
  climb(step, sublevel) {
    if (this.actionType === "climbing") {
      ...
      if (keys.up || keys.down) {
        this.imageSwap("climbing", this.spriteUpperBound);
      }
      const actorAt = sublevel.actorAt(this);
      if (actorAt && actorAt.type === "ladder") {
        const ladder = actorAt;
        const playerMountedTheLadder =
        ...
        if (onTheLeftSide || onTheRightSide) {
          if (keys.up || keys.down) {
            this.pos.x = ladder.coords.left - 3.1;
          }
        }
      }
    }
  ...
  removeDagger(sublevel, daggerToBeRemoved) {
    sublevel.player.daggers = sublevel.player.daggers.filter(
      dagger => dagger !== daggerToBeRemoved
    );
  }
  ...
  moveDaggers(step, sublevel) {
    ...
    sublevel.player.daggers.forEach(daggerCheck => {
      const obstacle = sublevel.obstacleAt(
        daggerCheck.pos,
        daggerCheck.size,
        this.buffer
      );
      if (obstacle) {
        if (obstacle.type === "wall") {
          this.removeDagger(sublevel, daggerCheck);
        }
      }
      const xDagger = daggerCheck.pos.x;
      const xPlayer = sublevel.player.pos.x;
      const movedFarAway = xDagger > xPlayer + 200 || xDagger < xPlayer - 200;
      if (movedFarAway) {
        this.removeDagger(sublevel, daggerCheck);
      }
    });
  ...
  removeSword(sublevel, swordToBeRemoved) {
    sublevel.player.swords = sublevel.player.swords.filter(
      sword => sword !== swordToBeRemoved
    );
  }
